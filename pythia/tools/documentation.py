...
project can move or be cloned elsewhere without invalidating the index). Files
whose hash is new or changed are fed to the default model
(config.DEFAULT_MODEL), which writes markdown docs mirroring the source tree —
each doc streams live in the terminal as it is written. Companion files sharing
the same path and stem (app.component.ts + app.component.html) are documented
together as one unit; stylesheets and markdown files are hashed but never
documented.

The generated docs are NOT saved to disk: their only purpose is the semantic
index. Each group's source files and freshly written doc are embedded with
config.EMBED_MODEL (sliced to fit its 2k-token context) directly into
.pythia/embeddings.json — the index consumed by the project_search tool. Like
index.json it is maintained incrementally: it is rewritten after every group,
and records belonging to groups that disappeared or changed are
dropped/replaced. A group is (re)documented whenever its source changes or it
has no records in the index yet.
...
(vendor/plantuml, bundled into frozen builds; needs java on PATH).
"""
...
def write_project_documentation() -> str:
    """Index the current folder and (re)embed docs for new/changed files.
    ...
    # The search index drops records of groups that no longer exist,
    # immediately, before any model call runs.
    ...
    gone = [key for key in old_groups if key not in new_groups]

    # A group is (re)documented when any member is new, changed, or removed,
    # or when it has no records in the search index yet.
    todo = sorted(
    ...
    console.print(
        ...
        f"{len(gone)} grupo(s) obsoleto(s) removido(s) do índice[/dim]"
    )

    # The index is persisted incrementally — an entry is updated the moment
    # its group is embedded — so an interrupted run resumes where it stopped
    ...
    lines = [
        f"Project search index updated under {root / PYTHIA_DIR}:",
        f"- {len(persisted)} file(s) hashed into {INDEX_FILENAME}",
        f"- {len(documented)} group(s) (re)documented and embedded",
        f"- {slices_embedded} text slice(s) embedded for semantic search",
        f"- {len(gone)} obsolete group(s) dropped from the index",
